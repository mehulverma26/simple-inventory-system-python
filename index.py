...
def Database():
    ...
    cursor.execute(
        "SELECT * FROM `admin` WHERE `username` = 'mehulverma26' AND `password` = 'mehulverma'"
    )
    if cursor.fetchone() is None:
        cursor.execute(
            "INSERT INTO `admin` (username, password) VALUES('mehulverma26', 'mehulverma')"
        )
        conn.commit()
...
def Exit():
    result = tkMessageBox.askquestion(
        "Simple Inventory System", "Are you sure you want to exit?", icon="warning"
    )
    if result == "yes":
        root.destroy()
        exit()
...
def LoginForm():
    ...
    lbl_text = Label(
        TopLoginForm, text="Administrator Login", font=("arial", 18), width=600
    )
    ...
    password = Entry(
        MidLoginForm, textvariable=PASSWORD, font=("arial", 25), width=15, show="*"
    )
    password.grid(row=1, column=1)
    btn_login = Button(
        MidLoginForm, text="Login", font=("arial", 18), width=30, command=Login
    )
...
def AddNewForm():
    ...
    productname = Entry(
        MidAddNew, textvariable=PRODUCT_NAME, font=("arial", 25), width=15
    )
    productname.grid(row=0, column=1)
    productqty = Entry(
        MidAddNew, textvariable=PRODUCT_QTY, font=("arial", 25), width=15
    )
    productqty.grid(row=1, column=1)
    productprice = Entry(
        MidAddNew, textvariable=PRODUCT_PRICE, font=("arial", 25), width=15
    )
...
def ViewForm():
    ...
    tree = ttk.Treeview(
        MidViewForm,
        columns=("ProductID", "Product Name", "Product Qty", "Product Price"),
        selectmode="extended",
        height=100,
...
def Logout():
    result = tkMessageBox.askquestion(
        "Simple Inventory System", "Are you sure you want to logout?", icon="warning"
    )
    if result == "yes":
        admin_id = ""
        root.deiconify()
